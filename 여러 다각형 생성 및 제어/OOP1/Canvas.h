...
public:
	...
	void drawLine(const char shape, const Vector2& start, const Vector2& end)
	{
		...
		Vector2 norDirVec = dir / dir.magnitude();

		float drawedLength = 0.0f;

		// draw the line until its length reach to diameter
		while (drawedLength <= dir.magnitude())
		{
			draw(&shape, Vector2{ startPos.x, startPos.y }, Dimension{ 1, 1 }, true);
			startPos += norDirVec;
			drawedLength = Vector2(startPos.x - start.x, startPos.y - start.y).magnitude();
		}
	}
